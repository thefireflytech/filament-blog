...
class PostForm
{
    public static function configure(Schema $schema, ?Category $category = null, ?Tag $tag = null): Schema
    {
        return $schema->components([
            Section::make(__('filament-blog.post.blog_details'))
                ->columnSpanFull()
                ->schema([
                    Fieldset::make(__('filament-blog.common.titles'))
                        ->schema([
                            Select::make('category_id')
                                ->label(__('filament-blog.category.categories'))
                            ...
                            Select::make('tag_id')
                                ->label(__('filament-blog.tag.tags'))
                    ...
                    Fieldset::make(__('filament-blog.post.feature_image'))
                        ->schema([
                            FileUpload::make('cover_photo_path')
                                ->label(__('filament-blog.post.cover_photo'))
                            ...
                            TextInput::make('photo_alt_text')
                                ->label(__('filament-blog.post.photo_alt_text'))
                                ->required(),
                        ])->columns(1),

                    Fieldset::make(__('filament-blog.post.status'))
                        ->schema([
