...
<div class="grid sm:grid-cols-2 gap-y-5 gap-x-10">
    <div class="h-[400px] w-full overflow-hidden rounded-xl bg-zinc-300">
        <img class="flex h-full w-full items-center justify-center object-cover object-top" src="{{ asset($post->featurePhoto) }}" alt="{{ $post->photo_alt_text }}">
    </div>
    <div class="flex flex-col justify-center space-y-10 py-4 sm:pl-10">
        <div>
            <div class="mb-5">
                <a href="{{ route('filamentblog.post.show', ['post' => $post->slug]) }}" class="hover:text-primary-700 mb-4 block text-4xl font-semibold hover:text-blue-600">
                    {{ $post->title }}
                </a>
                <div>
        ...
        <div class="flex items-center gap-4">
            <img class="h-14 w-14 overflow-hidden rounded-full bg-zinc-300 object-cover text-[0]" src="{{ $post->user->avatar }}" alt="{{ $post->user->name() }}">
            <div>
